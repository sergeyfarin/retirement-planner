...
| 4.7 | **Optimize Student-t generation** — ❌ Rejected / Kept Simple. The current $O(df)$ chained-normal method is computationally heavy, but an audit revealed it is only called exactly 120 times total per simulation run (to generate the initial bootstrap proxy pool). The millions of inner Monte Carlo loops just perform $O(1)$ array lookups against this pool. Therefore, converting to a Gamma-based algorithm would add massive code complexity for zero perceptible speedup. | M | `retirementEngine.ts` |
...
| 4.14 | **Log-Normal Outcome CDF Plot** — Add a dedicated Cumulative Distribution Function (CDF) plot for the *final simulated outcomes* (e.g., Year 30 Wealth), alongside the existing Timeline Plot. This will properly visualize the massive log-normal right-tail skew of compounded time, shifting focus away from just the narrow single-year assumption CDF. | M | `RetirementPlanner.svelte` |
