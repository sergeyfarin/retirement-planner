...
async function ensureWasm() {
	if (!wasmReady) {
		const wasmBuffer = readFileSync(
			join(process.cwd(), 'rust-engine', 'pkg', 'rust_engine_bg.wasm')
		);
...
function sampleMoments(values: number[]): {
	mean: number;
	std: number;
	skewness: number;
	kurtosis: number;
...
describe('coast FIRE age', () => {
	...
	it('is no later when the saver starts with more money', () => {
		...
		const poorer = runMonteCarloSimulation(
			saver({ currentSavings: 100_000 }),
			spending,
			bigSaver,
			[],
			months,
		...
		const richer = runMonteCarloSimulation(
			saver({ currentSavings: 300_000 }),
			spending,
			bigSaver,
			[],
			months,
...
describe('annual-mode intra-year variation', () => {
	...
	function annualOnlyInput(overrides: Partial<RetirementInput> = {}): RetirementInput {
		return {
			...
			historicalAnnualReturns: [
				...
				0.04, 0.13, -0.12, 0.1, 0.08, 0.09, 0.07, -0.18, 0.15, 0.1, 0.02, -0.05, 0.19, 0.11, -0.09
			],
	...
	it('leaves annual return distribution intact (returnVariability drives only intra-year spread)', () => {
		...
		const flat = runMonteCarloSimulation(
			annualOnlyInput({ returnVariability: 0 }),
			spending,
			[],
			[],
			months,
		...
		expect(spread.stats.returnMoments.arithmeticMean).toBeCloseTo(
			flat.stats.returnMoments.arithmeticMean,
			10
		);
	...
	it('exposes within-year ruin that a constant monthly rate would hide', () => {
		...
		const drawdownInput = annualOnlyInput({
			currentAge: 60,
			retirementAge: 60,
			simulateUntilAge: 61
		});
...
describe('portfolio kurtosis blending', () => {
	...
	function blendOf(stocks: number, bonds: number, bank: number, rho: number, stockKurt = 3) {
		return blendPortfolioMetrics(
			{
				...
				bondStd: 0.09,
				bondSkew: 0,
				bondKurt: normal(0.09).kurt,
				bankMean: 0.03,
				bankStd: 0.03,
				bankSkew: 0,
				bankKurt: normal(0.03).kurt
			},
			{ stocks, bonds, bank },
			rho
		);
...
describe('withdrawal strategies', () => {
	...
	it('omitting the strategy behaves like fixed spending', () => {
		const noStrategy = runMonteCarloSimulation(
			stressedInput(undefined),
			spending,
			[],
			[],
			months,
	...
	it('values delayed and temporary retirement income on its actual schedule', () => {
		...
		const delayed = runMonteCarloSimulation(
			input,
			spending,
			delayedPension,
			[],
		...
		const temporary = runMonteCarloSimulation(
			input,
			spending,
			temporaryIncome,
			[],
