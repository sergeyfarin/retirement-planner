...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **USD**   | [S&P 500 `^SPX`](https://stooq.com/q/d/?s=%5Espx) price index, plus estimated dividends                                                     | Synthetic 10Y total return from [`GS10`](https://fred.stlouisfed.org/series/GS10), duration 7y                              | [`TB3MS`](https://fred.stlouisfed.org/series/TB3MS)                     |
| **GBP**   | [FTSE 100 `^UKX`](https://stooq.com/q/d/?s=%5Eukx) price index, plus estimated dividends                                                    | Synthetic UK 10Y total return from [`IRLTLT01GBM156N`](https://fred.stlouisfed.org/series/IRLTLT01GBM156N), duration 7y     | [`IR3TIB01GBM156N`](https://fred.stlouisfed.org/series/IR3TIB01GBM156N) |
| **EUR**   | 60% [DAX](https://stooq.com/q/d/?s=%5Edax) + 40% [CAC](https://stooq.com/q/d/?s=%5Ecac), with a 3% estimated CAC dividend                   | Synthetic German 10Y total return from [`IRLTLT01DEM156N`](https://fred.stlouisfed.org/series/IRLTLT01DEM156N), duration 7y | Euro-area 3m rate stitched with German pre-euro                         |
| **WORLD** | 55% US + 15% EUR + 5% UK + 15% Japan (`^NKX`) + 10% Asia/EM (`^HSI`, backfilled with Nikkei). Every foreign equity leg is converted to USD. | Weighted US/UK/German 10Y bond returns                                                                                      | Average of US/UK/EUR cash rates                                         |

Regional CPI comes from FRED: [`CPIAUCSL`](https://fred.stlouisfed.org/series/CPIAUCSL)
for USD/WORLD, [`GBRCPIALLMINMEI`](https://fred.stlouisfed.org/series/GBRCPIALLMINMEI)
for GBP, and euro-area [`CP0000EZ19M086NEST`](https://fred.stlouisfed.org/series/CP0000EZ19M086NEST)
stitched to German [`DEUCPIALLMINMEI`](https://fred.stlouisfed.org/series/DEUCPIALLMINMEI)
before 1997. World equity FX conversion uses monthly FRED series
[`EXUSUK`](https://fred.stlouisfed.org/series/EXUSUK),
...
The three simulation-mode buttons in Advanced assumptions map onto the sampling modes below.
...
The "Use 2025–2026 yields" preset (§4.4.1) switches to **Historical (with Adjustments)**,
...
### 4.4.1 "Use 2025–2026 yields" — Embedded Current-Conditions Assumptions
...
- **Lump-sum events**: one-time addition/subtraction at a specific age, entered directly
...
| `taxOnGainsPercent` | 15%     | Applied annually to positive net investment P&L after monthly deflation and fees; losses are untaxed and not carried forward. This is a configurable drag on real gains, not a jurisdictional capital-gains-tax calculation. |

Annual P&L is tracked in the same deflated units as the balance, so tax is charged on
positive **real** investment gains. Most actual capital-gains systems tax realized nominal
gains and include account types, allowances, basis, disposal timing, and sometimes loss
carryforwards; none are modelled here. Treat the input as a rough return drag, not a tax
bill. Jurisdiction-specific modes (including Dutch Box 3) remain future work.
...
| `guardrails` spends all income when income ≥ planned spending                                                             | Surplus pension is consumed rather than invested, unlike `fixed` and `percentOfPortfolio`                                                              | Open — TODO 0.28. Not reachable from the default plan                                                                                                                                               |
...
| Ruin-surface spending multiplier scales the whole plan, accumulation included                                             | Intended: "spend less" is a change in standard of living from now on, not a retirement-only cut                                                         | The recommendation card should state the timeframe — TODO 0.26                                                                                                                                      |
